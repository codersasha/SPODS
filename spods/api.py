...
def serve_api(*args):
    ...
    # do any of these classes use cookie storage?
    for a in args:
        if hasattr(a, 'linkedclass') and a.session_field:
            # try and match with cookie values
            session_value = cookie.get(a.table.title + '_' + a.session_field)
            session_obj = a.get_one(**{ a.session_field: session_value })

            # force a new session object, if needed, and save back to the cookie
            if not session_obj and a.force_session:
                session_obj = a()
                cookie[a.table.title + '_' + a.session_field] = session_obj[a.session_field]
